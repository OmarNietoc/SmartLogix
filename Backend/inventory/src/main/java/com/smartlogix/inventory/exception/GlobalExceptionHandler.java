...
public class GlobalExceptionHandler {
    @ExceptionHandler({ProductNotFoundException.class, WarehouseNotFoundException.class, InventoryNotFoundException.class, InventoryReservationNotFoundException.class, ResourceNotFoundException.class})
    ...
    @ExceptionHandler({IllegalArgumentException.class, IllegalStateException.class, InsufficientStockException.class, BusinessException.class})
    ...
    public ResponseEntity<MessageResponse<Void>> handleDataIntegrity(DataIntegrityViolationException ex) {
        return new ResponseEntity<>(MessageResponse.<Void>builder().statusCode(HttpStatus.CONFLICT.value()).message("Recurso duplicado o conflicto de datos").data(null).build(), HttpStatus.CONFLICT);
    }
    @ExceptionHandler(Exception.class)
    public ResponseEntity<MessageResponse<Void>> handleGeneral(Exception ex) {
        return new ResponseEntity<>(MessageResponse.<Void>builder().statusCode(HttpStatus.INTERNAL_SERVER_ERROR.value()).message("Ocurrió un error interno en el servidor").data(null).build(), HttpStatus.INTERNAL_SERVER_ERROR);
    }
}
