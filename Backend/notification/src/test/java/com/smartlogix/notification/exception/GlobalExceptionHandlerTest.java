...
class GlobalExceptionHandlerTest {
    ...
    void handleResourceNotFound_returns404() {
        var response = handler.handleResourceNotFound(new ResourceNotFoundException("Notificacion no encontrada"));

        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND);
        assertThat(body(response).getMessage()).isEqualTo("Notificacion no encontrada");
    }

    @Test
    void handleValidation_returns400WithFieldDetail() {
        var response = handler.handleValidation(validationException("recipient", "must be a well-formed email address"));

        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.BAD_REQUEST);
        assertThat(body(response).getMessage()).isEqualTo("recipient: must be a well-formed email address");
    }
